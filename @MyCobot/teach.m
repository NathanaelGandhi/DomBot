...
function teach(myCobot, varargin)
    ...
    for k=j:j+size(axis,2)-1      % Continue counting from previous sliders
        % slider label
        uicontrol(panel2, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*(k-j+2) 0.15 height], ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.5, ...
            'String', sprintf(axis(:,(k-j+1))));
        
        % slider itself
        qCart(k-j+1) = max( qlimCart(k-j+1,1), min( qlimCart(k-j+1,2), qCart(k-j+1) ) ); % clip to range
        handles.slider(k) = uicontrol(panel2, 'Style', 'slider', ...
            'Units', 'normalized', ...
            'Position', [0.15 height*(k-j+2) 0.65 height], ...
            'Min', qlimCart(k-j+1,1), ...
            'Max', qlimCart(k-j+1,2), ...
            'Value', qCart(k-j+1), ...
            'Tag', sprintf('Slider%d', k));
        
        % text box showing slider value, also editable
        handles.edit(k) = uicontrol(panel2, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*(k-j+2)+.01 0.20 0.9*height], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(qscale(k-j+1)*qCart(k-j+1), 3), ...
            'HorizontalAlignment', 'left', ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.4, ...
            'Tag', sprintf('Edit%d', k));
    end
