...
function teach(myCobot, varargin)
    ...
    for k=1:numAxis                                         % Continue counting from previous sliders
        % slider label
        uicontrol(panel2, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*(numAxis-k+2) 0.15 height], ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.5, ...
            'String', sprintf(axis(:,(k))));
        
        % slider itself
        qCart(k) = max( qlimCart(k,1), min( qlimCart(k,2), qCart(k) ) ); % clip to range
        handles.slider(k) = uicontrol(panel2, 'Style', 'slider', ...
            'Units', 'normalized', ...
            'Position', [0.15 height*(numAxis-k+2) 0.65 height], ...
            'Min', qlimCart(k,1), ...
            'Max', qlimCart(k,2), ...
            'Value', qCart(k), ...
        ...
        handles.edit(k) = uicontrol(panel2, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*(numAxis-k+2)+.01 0.20 0.9*height], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(qscale(k)*qCart(k), 3), ...
            'HorizontalAlignment', 'left', ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.4, ...
