...
function teach(myCobot, varargin)
    ...
    set(ax, 'OuterPosition', [0.51 0 0.49 1])   % Limit for 2 panels
    ...
    %% ---- now make the sliders - Cartesian movements
    qCart = [0, 0, 0];      %q;
    qlimCart = [...
        myCobot.radiusOfMotion,-myCobot.radiusOfMotion;...
        myCobot.radiusOfMotion,-myCobot.radiusOfMotion;...
        myCobot.radiusOfMotion,-myCobot.radiusOfMotion;];   %qlim;
    
    %---- now make the sliders - Panel 2 Cartesian move
    for j=1:numAxis
        % slider label - Panel 2 Cartesian move
        uicontrol(panel2, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*2*(numAxis-j+2) 0.15 height*2], ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.5, ...
        ...
        % text box showing slider value, also editable - Panel 2 Cartesian move
        handles.edit2(j) = uicontrol(panel2, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*2*(numAxis-j+2)+.01 0.20 0.9*height*2], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(T6(j,4), 3), ...      % Use the end effector pose
            'HorizontalAlignment', 'left', ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.4, ...
            'Tag', sprintf('Edit%d', j));
    ...
    %% ---- now make the sliders - Panel 3 Object move
    for j=1:numAxis
        % slider label - Panel 3 Object move
        uicontrol(panel3, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*2*(numAxis-j+2) 0.15 height*2], ...
            'FontUnits', 'normalized', ...
        ...
        % slider itself - Panel 3 Object move
        q(j) = max( 1, min( -1) ); % clip to range - 1m to -1m
        handles.slider3(j) = uicontrol(panel3, 'Style', 'slider', ...
            'Units', 'normalized', ...
            'Position', [0.15 height*2*(numAxis-j+2) 0.65 height*2], ...
            'Min', qlim(j,1), ...
            'Max', qlim(j,2), ...
        ...
        % text box showing slider value, also editable - Panel 3 Object move
        handles.edit3(j) = uicontrol(panel3, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*2*(numAxis-j+2)+.01 0.20 0.9*height*2], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(1, 3), ...       % Set the object pose arbitary 1
            'HorizontalAlignment', 'left', ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.4, ...
            'Tag', sprintf('Edit%d', j));
