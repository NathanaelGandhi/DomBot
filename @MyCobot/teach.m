...
function teach(myCobot, varargin)
    ...
    axis = ['x','y','z'];
    for k=j:j+size(axis,2)      % Continue counting from previous sliders
        % slider label
        uicontrol(panel, 'Style', 'text', ...
            'Units', 'normalized', ...
            'BackgroundColor', bgcol, ...
            'Position', [0 height*(size(axis,2)-k+2) 0.15 height], ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.5, ...
            'String', sprintf(axis(:,k)));
        
        % slider itself
        q(k) = max( qlim(k,1), min( qlim(k,2), q(k) ) ); % clip to range
        handles.slider(k) = uicontrol(panel, 'Style', 'slider', ...
            'Units', 'normalized', ...
            'Position', [0.15 height*(size(axis,2)-k+2) 0.65 height], ...
            'Min', qlim(k,1), ...
            'Max', qlim(k,2), ...
            'Value', q(k), ...
        ...
        % text box showing slider value, also editable
        handles.edit(k) = uicontrol(panel, 'Style', 'edit', ...
            'Units', 'normalized', ...
            'Position', [0.80 height*(size(axis,2)-k+2)+.01 0.20 0.9*height], ...
            'BackgroundColor', bgcol, ...
            'String', num2str(qscale(k)*q(k), 3), ...
            'HorizontalAlignment', 'left', ...
            'FontUnits', 'normalized', ...
            'FontSize', 0.4, ...
