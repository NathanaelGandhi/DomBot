classdef Barrier < EnvironmentObject
    properties
        model;                      % Class object
        extinguisherVertexCount;    % Vertex count
    end
    
    methods%% Class for Fire Barrier simulation
        function self = Barrier(logArg, id, pose)
            % Call superclass constructor
            self = self@EnvironmentObject(logArg, id, pose, 'barrier');
            
            % Read ply file
            [tris,verts,data] = plyread('Barrier.ply','tri');
